...
export default function ContratsSection({ user: userProp }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        // 1. Récupérer l'user (prop ou session)
  ...
  if (loading) {
    return <div className="p-8 text-center text-gray-400">Chargement des contrats…</div>
  }
  ...
  return (
    <div className="mx-auto max-w-3xl p-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-900">Mes contrats</h2>
        <p className="mt-1 text-sm text-gray-500">
          Gérez vos contrats et activez les versements récurrents.
        </p>
      </div>
      ...
      {contracts.length === 0 ? (
        ...
        <div className="space-y-4">
          {contracts.map((c) => {
            ...
            const campaignTitle = c.collaborations?.campaigns?.title
              ...
              <div key={c.id} className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-semibold text-gray-900">
                      {campaignTitle || 'Contrat'}
                    </p>
                    <p className="text-sm text-gray-500">
                      {influencerName ? `Avec @${influencerName}` : 'Créateur'}
                    </p>
                  </div>
                  <span className="rounded-full bg-[#F3EFFF] px-3 py-1 text-xs font-semibold text-[#7C3AED]">
                ...
                <div className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-3">
                  <div>
                    ...
                    <p className="font-medium text-gray-900">{fmtEUR(c.amount)}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">{formatDate(c.deadline)}</p>
                  </div>
                </div>

                {/* Zone versements récurrents */}
                {isRecurring && (
                  <div className="mt-4 border-t border-gray-100 pt-4">
                    {hasSubscription ? (
                      ...
                      <div className="flex flex-col gap-2">
                        <p className="text-sm text-gray-500">
                          Versement de {fmtEUR(c.payout_amount)} {(FREQ_LABEL[c.payout_frequency] || c.payout_frequency).toLowerCase()} au créateur.
                        </p>
