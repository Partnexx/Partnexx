...
export default function PartnexxIntro({ ready = true }) {
  const [show, setShow] = useState(false) // rendu serveur = rien -> aucun mismatch d'hydratation
  ...
  useIso(() => {
    ...
    const tMax = setTimeout(() => { setFading(true); setTimeout(() => setShow(false), 800) }, 9000)
    return () => { clearTimeout(tMin); clearTimeout(tMax) }
  }, [])

  useEffect(() => {
    if (show && minDone && ready && !fading) {
      const buffer = setTimeout(() => { setFading(true); setTimeout(() => setShow(false), 800) }, 650)
      return () => clearTimeout(buffer)
    }
  }, [show, minDone, ready, fading])
