...
export function useAdminData() {
  ...
  const [dac7Report, setDac7Report] = useState([])

  const fetchDac7Report = useCallback(async (year = new Date().getFullYear()) => {
    try {
      const { data, error } = await supabase.rpc('get_dac7_annual_report', { report_year: year })
      if (error) throw error
      setDac7Report(data || [])
      return data || []
    } catch (err) {
      console.error('fetchDac7Report error:', err)
      setDac7Report([])
      return []
    }
  }, [])
