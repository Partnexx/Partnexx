...
/* ============================================================================
   Helper : calcule le niveau à partir d'un score
   ============================================================================ */
...
export function useUserLevel(userId) {
  ...
  // ===== DÉTECTION DU LEVEL-UP (via useEffect, en backup) =====
  useEffect(() => {
    ...
    if (currentLevelIndex > lastSeenIndex && currentLevelIndex > previousLevelIndexRef.current && level) {
      ...
      console.log(`🎉 Level up détecté (useEffect) ! ${lastSeenIndex} → ${currentLevelIndex} (${level.name})`)
  ...
  async function awardPoints(basePoints, reason, metadata = {}) {
    ...
    try {
      // 1. Update ai_score dans Supabase
      const { error: updateError } = await supabase
        .from('influencers')
        .update({ ai_score: newScore, updated_at: new Date().toISOString() })
        .eq('user_id', userId)

      if (updateError) throw updateError

      // 2. Update OPTIMISTE du state local (sans attendre le Realtime)
      setScore(newScore)
      setInfluencerData(prev => prev ? { ...prev, ai_score: newScore } : prev)

      // 3. ⭐ DÉTECTION DU LEVEL-UP À CHAUD (sans attendre le useEffect)
      if (level) {
        const newLevel = getLevelFromScore(newScore)
        const newLevelIndex = LEVELS.findIndex(l => l.key === newLevel.key)

        // Si on a passé un palier (newLevelIndex > currentLevelIndex)
        if (newLevelIndex > currentLevelIndex) {
          const storageKey = `partnexx-last-level:${userId}`
          const lastSeenIndex = parseInt(localStorage.getItem(storageKey) ?? '-2', 10)

          // Seulement si on n'a jamais vu ce niveau (ou supérieur)
          if (newLevelIndex > lastSeenIndex) {
            console.log(`🎉 Level up À CHAUD ! ${currentLevelIndex} → ${newLevelIndex} (${newLevel.name})`)

            // On déclenche le modal après un petit délai pour laisser le toast +XX points s'afficher d'abord
            setTimeout(() => {
              triggerLevelUp(newLevel, level)
      ...
      if (historyError) {
        console.warn('Score history log failed (points still awarded)', historyError)
      }

      // 5. Toast d'attribution des points
      if (multiplier > 1) {
        toast.success(
